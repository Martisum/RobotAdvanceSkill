...
template <class num1, class num2>
num1 abs_bound(num1 x, num2 range) {
	if (x < -range) x = -range;
	if (x > range) x = range;
	return (x);
}
...
template <class num>
num max_abs(num a, num b) { return ((fabs(a) > fabs(b)) ? a : b); }
template <class num>
...
template <class num>
num sign_nz(num x) { return ((x >= 0) ? 1 : -1); }
template <class num>
...
template <class real>
real fmodt(real x, real m) { return (x - floor(x / m) * m); }
...
template <class data>
int mcopy(data* dest, data* src, int num) {
	int i;
	for (i = 0; i < num; i++) dest[i] = src[i];
	return (num);
}
template <class data>
data mset(data* dest, data val, int num) {
	int i;
	for (i = 0; i < num; i++) dest[i] = val;
	return (val);
}
template <class data>
void mzero(data& d) { memset(&d, 0, sizeof(d)); }
template <class data>
void mzero(data* d, int n) { memset(d, 0, sizeof(data) * n); }
template <class node>
...
num angle_all(num x, num y) {
	...
	if (x == 0) {
		if (y >= 0) { return M_2PI / 4; }
		return M_2PI * 3 / 4;
	}
	return fabs(sign(y) * sign(x)) * ((1 - sign(y)) * (1 + sign(x)) / 4 * M_2PI + (1 + sign(y)) * (1 - sign(x)) / 8 * M_2PI + (1 -
		sign(y)) * (1 - sign(x)) / 8 * M_2PI) + (1 + sign(y)) * (1 - sign(y)) * (1 - sign(x)) / 4 * M_2PI + atan(y / x);
}
#endif
