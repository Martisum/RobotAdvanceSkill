...
namespace PlaneGeometry {
	template <class T>
	static void Swap(T& iT1, T& iT2) {
		T t = iT1;
		iT1 = iT2;
		iT2 = t;
	}
	template <class Num, typename = typename std::enable_if<std::is_arithmetic<Num>::value>::type>
	static Num Sign(Num iNum, bool iUseZero = false) { return iNum >= 0 ? iUseZero ? iNum > 0 : 1 : -1; }
	class Point {
	private:
		double _X{}, _Y{};
	public:
	...
	static double Dot(const Point& iPoint1, const Point& iPoint2) { return iPoint1.X() * iPoint2.X() + iPoint1.Y() * iPoint2.Y(); }
	static double Cross(const Point& iPoint1, const Point& iPoint2) { return iPoint1.X() * iPoint2.Y() - iPoint1.Y() * iPoint2.X(); }
	static double Calculate(const Point& iPoint, const Line& iLine) { return iLine.A() * iPoint.X() + iLine.B() * iPoint.Y() + iLine.C(); }
	static double Distance(const Point& iPoint1, const Point& iPoint2) { return (iPoint1 - iPoint2).Length(); }
	...
	static Point Symmetric(const Point& iPoint1, const Point& iPoint2) { return Point(2 * iPoint2.X() - iPoint1.X(), 2 * iPoint2.Y() - iPoint1.Y()); }
	static Point Symmetric(const Point& iPoint, const Line& iLine) {
		return Point(iPoint.X() - 2 * iLine.A() / Point(iLine.A(), iLine.B()).Length() * Distance(iPoint, iLine),
		             iPoint.Y() - 2 * iLine.B() / Point(iLine.A(), iLine.B()).Length() * Distance(iPoint, iLine));
	}
	static Line Symmetric(const Line& iLine, const Point& iPoint) { return Line(iLine.A(), iLine.B(), iLine.C() - 2 * Calculate(iPoint, iLine)); }
	//static Line Symmetric(const Line& iLine1, const Line& iLine2){}
}
#endif
