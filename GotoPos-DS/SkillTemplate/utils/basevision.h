...
public:
	...
	PlayerVision(const Robot& r, bool isPredict, bool isFound, bool valid)
		: isRobotPredict(isPredict), isExist(isFound), player(r) {}
	~PlayerVision() {}
	bool isRobotPredict;
	bool isExist;
	Robot player;
...
public:
	explicit AllocaArray() {
		for (int i = 0; i < MAXSIZE; i++) { p[i] = nullptr; }
		cnt = 0;
	}
	~AllocaArray() {}
