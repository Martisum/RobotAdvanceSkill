...
#define EPSILON (1.0E-10)
	...
	//=====================================================================//
	template <class num>
	class vector2d;  //使用类模板声明类存在（方便struct调用）
	template <class num>
...
#define VECTOR2D_LOGIC_OPERATOR(opr,combine) \
  ...
  {                            \
	...
	template <class num>
	vector2d<num> segment_near_line(const vector2d<num> a0, const vector2d<num> a1, const vector2d<num> b0,
	...
	template <class num>
	num vertex_angle(const vector2d<num> a, const vector2d<num> b, const vector2d<num> c) {
	...
	vector point_on_segment(const vector x0, const vector x1, const vector p,
	                        bool flag) {//flag = true；考虑线段的性质，flag = flase；不考虑线段的性质
		vector sx, sp, r;
		double f, l;
		sx = x1 - x0;
		sp = p - x0;
