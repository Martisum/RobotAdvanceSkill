...
class GameState {
	...
	int color;
public:
	GameState() : state(GAME_ON), isGameOver(false), color(BLUE) { }
	void init(int _color) { color = _color; }
	int get() const { return state; }
	void set(int _state) { state = _state; }

	// ball_kicked参数作用：执行中场开球和点球裁判盒指令时，还需要执行开球裁判盒指令比赛才能开始；但由于己方球员在跑位中场开球或者点球
	//阵型时碰撞到球，敌方认为比赛已经开始开始抢球，而己方还在等待裁判盒的开球指令才进入比赛。现象是：敌方已经在抢球进攻，己方待在原地不动。
	void transition(char ref_command, bool ball_kicked) {
		...
		if (ref_command == COMM_TIMEOUT_BLUE) { state = TIME_OUT | BLUE; }
		if (ref_command == COMM_TIMEOUT_YELLOW) { state = TIME_OUT | YELLOW; }
		...
		if (state == GAME_OFF) {
			switch (ref_command) {
				case COMM_KICKOFF_BLUE: state = KICKOFF | BLUE | NOTREADY;
					return;
				case COMM_KICKOFF_YELLOW: state = KICKOFF | YELLOW | NOTREADY;
					return;
				case COMM_PENALTY_BLUE: state = PENALTY | BLUE | NOTREADY;
					return;
				case COMM_PENALTY_YELLOW: state = PENALTY | YELLOW | NOTREADY;
					return;
				case COMM_DIRECT_BLUE: state = DIRECT | BLUE | READY;
					return;
				case COMM_DIRECT_YELLOW: state = DIRECT | YELLOW | READY;
					return;
				case COMM_INDIRECT_BLUE: state = INDIRECT | BLUE | READY;
					return;
				case COMM_INDIRECT_YELLOW: state = INDIRECT | YELLOW | READY;
				default: break;
			}
		}
	}
