...
public:
	...
	void set_task(int id, const PlayerTask& task) const {
		if(id < 0 || id > 11) return;
		task_list[id] = task;
	}
	const PlayerTask& get_task(int id) const {
		if(id < 0 || id > 11) return task_list[0];
		return task_list[id];
	}
	const bool arrive_point(int id, const point2f& current_pos, const float& dir) const {
		if(id < 0 || id > 11) return false;
		if((current_pos - task_list[id].target_pos).length() < arrive_point_range
			/*&&(fabs(anglemod(dir - task_list[id].orientate)))<5*PI/180*/) is_at_point[id] = true;
		else is_at_point[id] = false;
