...
namespace Maths {
	...
	inline point2f vector2polar(float length, float dir) { return point2f(length * cos(dir), length * sin(dir)); }
	...
	bool is_inside_penatly(const point2f& ball) {
		if (fabs(ball.x) > FIELD_LENGTH_H - PENALTY_AREA_R && ball.x < 0) {
			if (fabs(ball.y) < PENALTY_AREA_L / 2) return true;
			if (ball.y < 0 && (ball - FieldPoint::Goal_Penalty_Area_L_Left).length() < PENALTY_AREA_R) return true;
			if (ball.y > 0 && (ball - FieldPoint::Goal_Penalty_Area_L_Right).length() < PENALTY_AREA_R) return true;
			return false;
		}
		return false;
	}
